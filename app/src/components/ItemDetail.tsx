            ...
            <div className="space-y-4 animate-fade-in">
              <div className="glass-card p-4 rounded-2xl dynamic-container">
                ...
                {item.content_type === 'code' ? (
                  <div className="bg-black/50 rounded-xl overflow-x-auto border border-white/5 touch-pan-x [&_pre]:!overflow-x-auto [&_pre]:!max-w-full [&_pre_code]:!whitespace-pre">
                    <MarkdownRenderer content={`\`\`\`\n${item.content}\n\`\`\``} />
                  </div>
                ) : (
                  <div className="prose prose-invert prose-sm max-w-none prose-headings:text-white prose-headings:font-bold prose-p:text-remembra-text-secondary prose-p:leading-relaxed prose-a:text-remembra-accent-primary prose-strong:text-white prose-code:text-remembra-accent-secondary prose-code:bg-black/50 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded [&_pre]:!overflow-x-auto [&_pre]:touch-pan-x [&_pre]:!max-w-[calc(100vw-4rem)] [&_pre_code]:!whitespace-pre">
